...
class Ranges
  ...
  aattr_initialize :range_attrs, %i[range_start range_end]
  ...
  validates_each :range_start, :range_end do |record, attr, value|
    if value.present? && value !~ /^[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])/
      record.errors.add(attr, 'incorrect date')
    end
  end
